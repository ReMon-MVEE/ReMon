...
struct pa_mempool {
    ...
    pa_atomic_ptr_t* data_directory;
    int head;
    int directory_size;
    pa_mutex *directory_mutex;
...
int pa_get_index_for_pool(pa_mempool *p, void* ptr);
...
static pa_memblock *memblock_new_appended(pa_mempool *p, size_t length) {
    ...
    pa_atomic_store(&b->data, pa_get_index_for_pool(p, (uint8_t*) b + PA_ALIGN(sizeof(pa_memblock))));
    b->length = length;
...
/* No lock necessary */
static unsigned mempool_slot_idx(pa_mempool *p, int ptr_index) {
    pa_assert(p);

    void* temp_ptr = pa_atomic_ptr_load(&p->data_directory[ptr_index]);

    pa_assert((uint8_t*) temp_ptr >= (uint8_t*) p->memory.ptr);
    pa_assert((uint8_t*) temp_ptr < (uint8_t*) p->memory.ptr + p->memory.size);

    return (unsigned) ((size_t) ((uint8_t*) temp_ptr - (uint8_t*) p->memory.ptr) / p->block_size);
}

/* No lock necessary */
static struct mempool_slot* mempool_slot_by_ptr(pa_mempool *p, int ptr_index) {
    unsigned idx;

    if ((idx = mempool_slot_idx(p, ptr_index)) == (unsigned) -1)
        return NULL;
...
pa_memblock *pa_memblock_new_pool(pa_mempool *p, size_t length) {
    ...
    if (p->block_size >= PA_ALIGN(sizeof(pa_memblock)) + length) {
        ...
        pa_atomic_store(&b->data, pa_get_index_for_pool(p, mempool_slot_data(slot)));
...
pa_memblock *pa_memblock_new_fixed(pa_mempool *p, void *d, size_t length, bool read_only) {
    ...
    b->is_silence = false;
    pa_atomic_store(&b->data, pa_get_index_for_pool(p, d));
    b->length = length;
    pa_atomic_store(&b->n_acquired, 0);
    pa_atomic_store(&b->please_signal, 0);
...
void* pa_memblock_acquire(pa_memblock *b) {
    ...
    return pa_atomic_ptr_load(&b->pool->data_directory[pa_atomic_load(&b->data)]);
}
...
static void memblock_free(pa_memblock *b) {
    ...
    switch (b->type) {
        ...
        case PA_MEMBLOCK_POOL: {
            ...
            pa_assert_se(slot = mempool_slot_by_ptr(b->pool, pa_atomic_load(&b->data)));
...
static void memblock_make_local(pa_memblock *b) {
    ...
    if (b->length <= b->pool->block_size) {
        ...
        if ((slot = mempool_allocate_slot(b->pool))) {
            ...
            memcpy(new_data, pa_atomic_ptr_load(&b->pool->data_directory[pa_atomic_load(&b->data)]), b->length);
            pa_mutex_lock(b->pool->directory_mutex);
            pa_atomic_ptr_store(&b->pool->data_directory[pa_atomic_load(&b->data)], new_data);
            pa_mutex_unlock(b->pool->directory_mutex);
    ...
    void* new_data = pa_xmemdup(pa_atomic_ptr_load(&b->pool->data_directory[pa_atomic_load(&b->data)]), b->length);
    pa_atomic_store(&b->data, pa_get_index_for_pool(b->pool, new_data));
    b->per_type.user.free_cb_data = new_data;
...
pa_mempool *pa_mempool_new(pa_mem_type_t type, size_t size, bool per_client) {
    ...
    p->directory_size = 1024;
    p->data_directory = malloc(p->directory_size * sizeof(pa_atomic_ptr_t));
    memset(p->data_directory, -1, p->directory_size);
    p->head = 0;
    p->directory_mutex = pa_mutex_new(true, true);
...
pa_memblock* pa_memimport_get(pa_memimport *i, pa_mem_type_t type, uint32_t block_id, uint32_t shm_id,
    ...
    b->is_silence = false;
    pa_atomic_store(&b->data, pa_get_index_for_pool(b->pool, (uint8_t*) seg->memory.ptr + offset));
    b->length = size;
    pa_atomic_store(&b->n_acquired, 0);
    pa_atomic_store(&b->please_signal, 0);
...
static pa_memblock *memblock_shared_copy(pa_mempool *p, pa_memblock *b) {
    ...
    memcpy(pa_atomic_ptr_load(&n->pool->data_directory[pa_atomic_load(&n->data)]),
            pa_atomic_ptr_load(&b->pool->data_directory[pa_atomic_load(&b->data)]),
            b->length);
    return n;
}
...
int pa_get_index_for_pool(pa_mempool *p, void* ptr)
{
    pa_mutex_lock(p->directory_mutex);

    for (int i = 0; i < p->directory_size; i++)
    {
        if (pa_atomic_ptr_cmpxchg(&p->data_directory[i % p->directory_size], (void*) -1, ptr))
        {
            // printf(" > added at index %d\n", i);
            pa_mutex_unlock(p->directory_mutex);
            return i;
        }
    }

    p->head = p->directory_size;
    p->directory_size += 1024;
    pa_atomic_ptr_t* new_data_directory = malloc(p->directory_size * sizeof(pa_atomic_ptr_t));

    memcpy(new_data_directory, p->data_directory, p->directory_size);
    memset(new_data_directory + p->directory_size - 1024, -1, 1024);
    p->data_directory = new_data_directory;

    if (!pa_atomic_ptr_cmpxchg(&p->data_directory[p->head], (void*) -1, ptr))
        exit(-1);

    // printf(" > added at index %d (head)\n", p->head);
    pa_mutex_unlock(p->directory_mutex);
    return p->head++;
}
