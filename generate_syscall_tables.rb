...
File.open("MVEE/Src/MVEE_syscalls_handlers.cpp").each { |line|
  if line.match(/^\s*(long|DONTNEED)\s*monitor::handle_[[:graph:]]*_(get_call_type|precall|log_args|log_return|postcall|call)(\s|\()/)
    syscall     = line.gsub(/^\s*(long|DONTNEED)\s*monitor::handle_/, "").gsub(/_(get_call_type|precall|log_args|log_return|postcall|call)(\s|\()(.|\n)*/, "")
    handlertype = line.match(/_(get_call_type|precall|log_args|log_return|postcall|call)(\s|\()/)[1]
    notneeded   = line.match(/DONTNEED/)
    handlerfunc = line.gsub(/^\s*(long|DONTNEED)\s*monitor::/, "").gsub(/(\s|\().*/, "")
...
File.open("#{@arch}/MVEE_syscall_handler_table.tmp", 'w') { |file|
  ...
  file.write("};\n")
  file.write("const mvee_syscall_handler monitor::syscall_logger_table[MAX_CALLS][2] = {\n")
  cnt=0
  @syscalls.each { |num, name|
    ...
    write_handler(file, name, "log_args", "MVEE_HANDLER_DONTNEED")
    write_handler(file, name, "log_return", "MVEE_HANDLER_DONTNEED")
    write_footer(file)
...
File.open("#{@arch}/MVEE_syscall_handler_prototypes.tmp", 'w') { |file|
  ...
  @handlers.each { |syscall, funcs|
    funcs.each { |type, func|
      file.write("long #{func}(int childnum);\n") if not func.match(/DONTNEED/)
    }
  }
}
